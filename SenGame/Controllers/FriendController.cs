    ...
    public class FriendController : Controller
    {
        ...
        public IActionResult Chat(int id=1)
        {
            var User = from u in user
                       join ug in usergroup on u.UserId equals ug.UserId
                       join fg in friendgroup on ug.FriendGroupId equals fg.FriendGoupId
                       select new { u.UserId,u.UserName, u.UserPicture, fg.GroupName };
            var AllFriends = User.Where(x => x.UserId != id).Select(x => new { x.UserName, x.UserPicture, x.GroupName });
